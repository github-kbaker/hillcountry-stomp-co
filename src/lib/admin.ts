...
export const sendTestEmail = createServerFn({ method: "POST" }).handler(async () => {
  ...
  let result = await sendEmail({ to: recipient, subject, body: "This is a test email generated by the production website.", replyTo: process.env.BUSINESS_EMAIL || "" });
  ...
  if (!result.ok && result.error !== "RESEND_API_KEY not configured") {
    result = await sendEmail({ to: recipient, subject, body: "This is a test email generated by the production website.", replyTo: process.env.BUSINESS_EMAIL || "" });
